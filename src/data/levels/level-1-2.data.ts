...
export const level1_2MediumQuestions: Question[] = [
  ...
  // 第 5 题：组合图形识别（救援直升机场景）- 救护车拼图
  {
    ...
    // 使用绝对定位布局
    useAbsoluteLayout: true,
    layoutSize: { width: 570, height: 300 },
    targets: [
      // 警灯 - 车顶上方中央
      { id: 'light-pos', name: '警灯', accepts: ['light'], position: { x: 180, y: 20 }, size: { width: 60, height: 50 } },
      // 车身 - 中间主体长方形
      { id: 'body-pos', name: '车身', accepts: ['body'], position: { x: 85, y: 80 }, size: { width: 400, height: 100 } },
      // 窗户 - 车身前部（驾驶室）
      { id: 'window-pos', name: '窗户', accepts: ['window'], position: { x: 90, y: 100 }, size: { width: 80, height: 60 } },
      // 前轮 - 左下方
      { id: 'wheel1-pos', name: '前轮', accepts: ['wheel1', 'wheel2'], group: 'wheel', position: { x: 120, y: 185 }, size: { width: 80, height: 80 } },
      // 后轮 - 右下方
      { id: 'wheel2-pos', name: '后轮', accepts: ['wheel1', 'wheel2'], group: 'wheel', position: { x: 370, y: 185 }, size: { width: 80, height: 80 } },
    ],
