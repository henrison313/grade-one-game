...
**Goal:** 实现预录制音频优先播放，Web Speech API 和百度 TTS 作为降级兜底方案

**Architecture:** 扩展 speech.service.ts，新增 PRECORDED 模式作为最高优先级。保留百度 TTS 服务。音频文件按关卡分文件夹存放，进入关卡时按需加载。
...
| 文件 | 操作 | 职责 |
|------|------|------|
| `src/services/baidu-tts.service.ts` | 保留 | 百度云端 TTS 兜底 |
| `src/services/index.ts` | 保留 | 无需修改 |
| `src/services/speech.service.ts` | 修改 | 核心服务：新增预录制音频支持、调整优先级 |
| `src/features/story/story-player/story-player.component.tsx` | 修改 | 传入关卡 ID、预加载音频、传入片段索引 |
| `public/audio/story/` | 新建 | 预录制音频文件目录 |

---

### Task 1: 更新 TTSMode 枚举

**Files:**
- Modify: `src/services/speech.service.ts`

- [ ] **Step 1: 更新 TTSMode 枚举顺序**
...
git commit -m "feat(speech): update tts mode enum with precorded as priority"
```
...
- [ ] **Step 2: 更新 performWebSpeechTest 方法中的超时检测**

修改 `performWebSpeechTest` 方法中的超时检测逻辑（约第 157-171 行）：
...
```bash
git add src/services/speech.service.ts
git commit -m "feat(speech): update tts mode priority to precorded first"
```
...
- [ ] **Step 1: 更新 speak 方法签名和分支逻辑**

修改 `speak` 方法（约第 513-533 行）：
...
- [ ] **Step 2: 更新 speakAsync 便捷函数**

修改文件末尾的 `speakAsync` 函数：
...
```bash
git add src/services/speech.service.ts
git commit -m "feat(speech): update speak method to support precorded audio priority"
```

---

### Task 9: 更新 StoryPlayer 组件
