...
import { level41QuestionsByDifficulty } from '../src/data/levels/level-4-1-enhanced.data'
import { level42QuestionsByDifficulty } from '../src/data/levels/level-4-2-enhanced.data'
import { level43QuestionsByDifficulty } from '../src/data/levels/level-4-3-enhanced.data'
...
function validateMathCalculation(q: Question, level: string, difficulty: string, index: number) {
  ...
  const patterns = [
    ...
    // 带文字的: 等于多少, 是几
    { regex: /(\d+)\s*\+\s*(\d+)[^\d]*(?:等于|是|为)[^\d]*(\d+)/, type: 'add2_verify' },
    { regex: /(\d+)\s*-\s*(\d+)[^\d]*(?:等于|是|为)[^\d]*(\d+)/, type: 'sub2_verify' },
  ]
  ...
  for (const pattern of patterns) {
    ...
    if (pattern.type === 'add2') {
      ...
      calculated = parseInt(match[1]) + parseInt(match[2]) - parseInt(match[3])
    } else if (pattern.type === 'add2_verify' || pattern.type === 'sub2_verify') {
      // 验证题目中的答案是否正确
      const num1 = parseInt(match[1])
      const num2 = parseInt(match[2])
      const givenAnswer = parseInt(match[3])
      calculated = pattern.type === 'add2_verify' ? num1 + num2 : num1 - num2
      if (calculated !== givenAnswer) {
        addError(level, difficulty, index, q.type, `题目中的计算错误: ${num1} ${pattern.type === 'add2_verify' ? '+' : '-'} ${num2} = ${calculated}，但题目给出的是 ${givenAnswer}`)
      }
      continue
    } else {
      continue
    ...
    if (q.type === QuestionType.CHOICE && q.options && q.correctAnswer) {
      ...
      if (correctOption) {
        ...
        if (!isNaN(optionNum) && optionNum !== calculated) {
          addError(level, difficulty, index, q.type, `答案计算错误: ${match[1]} ${match[2] && match[3] ? (pattern.type.includes('add') ? '+' : '-') : ''} ${match[2]} = ${calculated}，但正确答案是 ${optionNum}`)
        }
      }
    }
 ...
 * 主题一致性检查
 */
function validateThemeConsistency(q: Question, level: string, difficulty: string, index: number, theme: string) {
  const text = q.question.toLowerCase()

  // 关卡 5-1: 不进位加法
  if (theme === 'no_carry_addition') {
    // 检查是否有进位的情况
    const addPattern = /(\d)\s*\+\s*(\d)/g
    let match
    while ((match = addPattern.exec(text)) !== null) {
      const digit1 = parseInt(match[1])
      const digit2 = parseInt(match[2])
      if (digit1 + digit2 >= 10) {
        addWarning(level, difficulty, index, q.type, `不进位加法主题下出现个位进位: ${digit1} + ${digit2} = ${digit1 + digit2}`)
      }
    }
  }

  // 关卡 5-2: 不退位减法
  if (theme === 'no_borrow_subtraction') {
    // 检查是否有退位的情况
    ...
    while ((match = subPattern.exec(text)) !== null) {
      ...
      const digit2 = parseInt(match[2])
      if (digit1 < digit2) {
        addWarning(level, difficulty, index, q.type, `不退位减法主题下出现个位借位: ${digit1} - ${digit2} 需要借位`)
      }
    }
  }

  // 关卡 4-3: 不进位不退位
  if (theme === 'no_carry_no_borrow') {
    // 检查加法
    const addPattern = /(\d)\s*\+\s*(\d)/g
    let match
    while ((match = addPattern.exec(text)) !== null) {
      ...
      const digit2 = parseInt(match[2])
      if (digit1 + digit2 >= 10) {
        addWarning(level, difficulty, index, q.type, `不进位不退位主题下出现进位: ${digit1} + ${digit2} = ${digit1 + digit2}`)
      }
    }
    // 检查减法
    const subPattern = /(\d)\s*-\s*(\d)/g
    while ((match = subPattern.exec(text)) !== null) {
      ...
      const digit2 = parseInt(match[2])
      if (digit1 < digit2) {
        addWarning(level, difficulty, index, q.type, `不进位不退位主题下出现退位: ${digit1} - ${digit2} 需要借位`)
      }
    }
  }
}
...
const levels: LevelConfig[] = [
  { id: '4-1', name: '炫蓝闪电S', data: level41QuestionsByDifficulty },
  { id: '4-2', name: '焰龙战神', data: level42QuestionsByDifficulty },
  { id: '4-3', name: '爆旋洛克', data: level43QuestionsByDifficulty, theme: 'no_carry_no_borrow' },
  { id: '5-1', name: '猎空悍将', data: level51QuestionsByDifficulty, theme: 'no_carry_addition' },
  { id: '5-2', name: '钢臂力士', data: level52QuestionsByDifficulty, theme: 'no_borrow_subtraction' },
]
...
function validateLevel(level: LevelConfig) {
  ...
  for (const diff of difficulties) {
    ...
    questions.forEach((q, i) => {
      ...
      // 主题一致性验证
      if (level.theme) {
        validateThemeConsistency(q, level.id, diff.name, i, level.theme)
      }
    })
  }
}
